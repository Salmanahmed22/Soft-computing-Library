...
public class CaseStudyApplication {
    ...
    public static Population evolve(Population population, Selection selection,
        ...
        return new Population(newPopulationChromosomes);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < generations; i++) {
            ...
            long genTimeMs = Duration.between(genStart, genEnd).toMillis();

            printPopulation(population, i + 1);
            System.out.printf("Generation %d runtime: %.3f seconds%n", i + 1, genTimeMs / 1000.0);
        }


        Instant totalEnd = Instant.now();
        long totalTimeMs = Duration.between(totalStart, totalEnd).toMillis();
        ...
        System.out.printf("Total Runtime Performance: %.3f seconds (%.0f ms)%n",
                totalTimeMs / 1000.0, (double) totalTimeMs);
        System.out.println("=".repeat(80));
